...
## User Flow
1. User runs `unblink-node.exe` interactively once.
...
- Add a setup mode flag, e.g. `unblink-node setup` (or `-setup`).
- Keep normal `unblink-node` runtime behavior unchanged.
