...
export default function ChatInput() {
  ...
  onMount(() => {
    ...
    const handleFocus = () => textareaRef?.focus();
    window.addEventListener("chat-focus-input", handleFocus);
    return () => window.removeEventListener("chat-focus-input", handleFocus);
  });

  createEffect(() => {
    if (inputValue() === "" && textareaRef) {
  ...
  return (
    <div class="px-4 pb-2 ">
      <div class="max-w-4xl mx-auto lg:px-2">
        <div
          data-ta-state={isTextareaFocused() ? "focused" : "blurred"}
          class="w-full rounded-3xl p-3 transition-all duration-150 focus-within:border-neu-750 bg-neu-800 shadow-lg"
        >
